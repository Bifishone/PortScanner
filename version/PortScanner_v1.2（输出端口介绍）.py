...
def load_port_descriptions(filename: str) -> Dict[int, str]:
    ...
    for encoding in encodings:
        try:
            ...
            print(f"{Fore.YELLOW}警告: 端口描述文件 {filename} 不存在，将不显示端口描述")
            return {}
        except Exception as e:
            print(f"{Fore.YELLOW}警告: 读取端口描述文件时出错: {str(e)}，将不显示端口描述")
            return {}

    # 如果所有编码都尝试失败
    print(f"{Fore.YELLOW}警告: 无法解析端口描述文件 {filename}，将不显示端口描述")
    return {}
...
def scan_ips_ports(ips: List[str], ports: List[int], port_descriptions: Dict[int, str],
                   timeout: float = 3.0, threads: int = 5) -> None:
        ...
        while not task_queue.empty():
            ...
            try:
                ...
                with print_lock:
                    ...
                    if is_open:
                        # 获取端口描述，如果没有则为空
                        port_desc = port_descriptions.get(port, "")
                        # 格式化输出，三列严格对齐：
                        # 第一列：IP:端口（左对齐，宽度30）
                        # 第二列：端口描述（左对齐，宽度40）
                        # 第三列：状态（右对齐，宽度20）
                        print(f"\n{Fore.WHITE}{ip}:{port:<30} {Fore.CYAN}{port_desc:<40} {Fore.GREEN}{status:>20}")
    ...
    for ip in open_ports:
        if open_ports[ip]:
            ...
            for port in open_ports[ip]:
                ...
                if desc:
                    # 端口号左对齐（宽度5），后接括号包裹的描述
                    port_info.append(f"{port:<5}({desc})")
                else:
                    port_info.append(f"{port:<5}")
            # 用逗号拼接成整齐的列表
...
def main():
    ...
    try:
        ...
        # 执行扫描，传入端口描述字典
        scan_ips_ports(ips, ports, port_descriptions, args.timeout, args.threads)
